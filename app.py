...
def giris():
    if request.method == "POST":
        ...
        if kullanici:
            ...
            return render_template("giris.html", hata="Hatalı kullanıcı adı veya şifre")
...
def dusukstok():
    ...
    with sqlite3.connect("stok.db") as con:
        ...
        # 🔴 Düşük stok kontrolü (eşik: 5)
        cur.execute("SELECT * FROM urunler WHERE miktar <= 5")
        dusuk_stoklar = cur.fetchall()

    return render_template("index.html", urunler=urunler, q=q, dusuk_stoklar=dusuk_stoklar)
